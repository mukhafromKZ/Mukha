from tkinter import *
from tkinter import messagebox #уведомление

cal = Tk()                      # Создание базового окна
cal.geometry("312x394")         # Размер окна: ширина:312 высота:394
cal.resizable(0, 0)             # не меняй размера окна 
cal.title("Calculator")

#работа

def about():
    messagebox.showinfo('About'," \n Created by Duman Mukhamedzhan ")

def click_button(item):                                           # Функция click_button постоянно обновляет поле ввода, когда вы вводите число
    global expression
    inputText.set(inputText.get()+(str(item)))

def clear_button():                                               #Функция clear_button очищает последнее что написали
    global expression
    expression = ""
    inputText.set(inputText.get()[0:-1])

def clear_all():                                                  # clear_all ну здесь все понятно
    inputText.set("")

def equal_button():
    result = ""
    try:
        result = eval(inputText.get())                             # eval равно
        inputText.set(result)
    except:
        result = "ERROR/ТАК НЕЛЬЗА"
        inputText.set(result)
...
#Frame
expression = ""
inputText = StringVar()                     # StringVar() кнопки цвета размеры
...
cal.mainloop()                       #mainloop чтоб мега код работал всегда